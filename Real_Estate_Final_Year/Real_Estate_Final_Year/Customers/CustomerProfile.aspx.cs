...
namespace Real_Estate_Final_Year.Customers
{
    public partial class CustomerProfile : System.Web.UI.Page
    {
        ...
        {
            // if (Session["CustUsrname"]!= null)
            // {
            try
            {
                SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["RealEstateConn"].ToString());

                var chkusr = "select * from UserRegistration where Username ='" + (string)Session["CustUsrname"] + "'";
                con.Open();
                ...
                if (dr.Read())
                {
                    ...
                    Session["CustomerName"] = dr["FirstName"].ToString();
                }
                else
                {
